...
// Placeholder search index — replace with real search infrastructure later.
// Matches user input against known routes/sections and navigates to the
// best match, since there's no search backend yet.
const SITE_INDEX = [
  { label: "Home", path: "/", keywords: ["home", "start"] },
  { label: "Features", path: "/#features", keywords: ["features", "capabilities"] },
  { label: "How it works", path: "/#how-it-works", keywords: ["how it works", "clock in", "clock-in"] },
  { label: "Industries", path: "/#industries", keywords: ["industries", "field services", "security", "retail", "logistics", "manufacturing"] },
  { label: "FAQ", path: "/#faq", keywords: ["faq", "questions", "help"] },
  { label: "Contact", path: "/#contact", keywords: ["contact", "pilot", "sales"] },
  { label: "Log in", path: "/login", keywords: ["login", "log in", "sign in"] },
  { label: "Sign up", path: "/login?mode=sign-up", keywords: ["sign up", "signup", "register", "start free"] },
];
...
export function NotFound() {
  ...
  function handleSearch(e: React.FormEvent) {
    ...
    if (match) {
      ...
      // No confident match — send them home rather than a dead end.
      router.push("/");
    }
  }
    ...
    <div className="h-screen w-full flex items-center justify-center py-16 px-4 md:py-24 md:px-20">
      ...
      <div className="z-10 flex flex-col items-center justify-center gap-8 md:gap-12">
        ...
        <form
          ...
          className="flex gap-4 flex-col md:flex-row w-full items-center justify-center"
        >
          ...
          <Button className="w-full md:w-fit" variant="outline" type="submit">
            Search
          </Button>
        </form>
